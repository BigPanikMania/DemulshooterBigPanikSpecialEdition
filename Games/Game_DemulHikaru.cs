...
namespace DemulShooter
{
    ...
    {
        public Game_DemulHikaru(String Rom, String DemulVersion, double ForcedXratio, bool DisableInputHack, bool Verbose, bool DisableWindow, bool WidescreenHack)
            : base(Rom, "hikaru", DemulVersion, ForcedXratio, DisableInputHack, Verbose, DisableWindow, WidescreenHack)
        {}
        ...
        protected override void CreateOutputList()
        {
            ...
            if (_RomName.Equals("swracer"))
            {
                _Outputs = new List<GameOutput>();
                _Outputs.Add(new GameOutput(OutputDesciption.LmpStart, OutputId.LmpStart));
                _Outputs.Add(new GameOutput(OutputDesciption.LmpBoost, OutputId.LmpBoost));
                _Outputs.Add(new GameOutput(OutputDesciption.LockBoost, OutputId.LockBoost));
        ...
        private void Compute_swracer_Outputs()
        {
            //Genuine Outputs
            SetOutputValue(OutputId.LmpStart, ReadByte(0x007000C4) >> 7 & 0x01);
            SetOutputValue(OutputId.LmpBoost, ReadByte(0x007000C4) >> 4 & 0x01);
            SetOutputValue(OutputId.LockBoost, ReadByte(0x007000C4) >> 3 & 0x01);
        }
        #endregion
    }    
}
